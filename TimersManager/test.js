...
class TimersManager {
  ...
  start() {
    ...
    this.timers.map(({ timer, args }, key) => {
      timer.interval ?
        global[timer.name] = this.startedTimers[timer.name] = setInterval(timer.job, timer.delay, ...args) :
        global[timer.name] = this.startedTimers[timer.name] = setTimeout(timer.job, timer.delay, ...args);
  ...
  resume(name) {
    ...
    const { delay, interval } = this.startedTimers[name];
    // this.timers
    return this;
  }
